...
/**
 * Vite plugin that strips the `crossorigin` attribute and `type="module"` from the final HTML output,
 * and adds `defer` to <script> so it runs after DOM is parsed (since we removed the implicit defer from module).
 * Under file:// protocol (Wallpaper Engine), crossorigin triggers CORS errors
 * and ES modules are blocked from file:// in modern Chrome.
 */
...
export default defineConfig(() => {
  return {
    // Use relative paths so assets resolve correctly under file:// protocol (Wallpaper Engine)
    base: './',
    plugins: [react(), tailwindcss(), neteaseApiPlugin(), stripCrossoriginPlugin()],
    build: {
      // Bundle everything into a single IIFE script — works under file:// (Wallpaper Engine / offline)
      rollupOptions: {
        output: {
          format: 'iife',
