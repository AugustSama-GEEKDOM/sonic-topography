...
import { MediaInfoPanel, type MediaInfo } from './MediaInfoPanel';
import { extractLyricsFromAudio } from '../../lib/metadata';

interface UIProps {
  theme: string;
  onThemeChange: (theme: string) => void;
  isMediaIntegrationEnabled?: boolean;
...
export function UI({ theme, onThemeChange, isMediaIntegrationEnabled, mediaInfo }: UIProps) {
  ...
  }, [playlists, isFileProtocol]);

  useEffect(() => {
    const loadPlaylists = async () => {
      if (isFileProtocol) {
        // Running from file:// — use localStorage only; no server available
  ...
  return (
    ...
    >
      ...
      {/* Media Info Panel — shows currently playing track info from Wallpaper Engine */}
      <MediaInfoPanel
        info={(mediaInfo?.title || mediaInfo?.artist) ? mediaInfo : null}
        accentHex={accentHex}
      />
      ...
      {trackName !== 'No track selected' && (
        <div className="absolute top-[40px] right-[40px] w-[300px] p-6 rounded-sm z-50 pointer-events-auto backdrop-blur-[20px] border border-white/10" style={{ background: 'rgba(255,255,255,0.03)' }}>
          ...
          <div className="text-[12px] opacity-50 uppercase mb-6 tracking-wider">
             ...
             <span className="ml-2 text-[10px]" style={{ color: theme === 'dynamic' ? accentHex : '#3b82f6' }}>
               &bull; {theme === 'dynamic' && isMediaIntegrationEnabled ? 'Dynamic \u266B' : themes[theme]?.name}
             </span>
          </div>
