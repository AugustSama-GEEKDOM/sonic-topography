...
export class AudioEngine {
  ...
  // Wallpaper Engine system audio bridge
  private usingWallpaperAudio = false;
  private wallpaperSilenceFrames = 0;
  private readonly wallpaperSilenceThreshold = 120; // ~2 seconds at ~60fps
  ...
  /**
   * Feed system audio data from Wallpaper Engine's wallpaperRegisterAudioListener.
   *
   * The audioArray has 128 elements:
   *   0-63:   left channel frequencies (bass → treble)
   *   64-127: right channel frequencies (bass → treble)
   * Values are typically 0.0 – 1.0 (occasionally higher).
   */
  public feedWallpaperAudio(audioArray: number[]) {
    if (!audioArray || audioArray.length < 128) return;

    // Ensure dataArray is allocated (512 bins, matching fftSize=1024)
    if (this.dataArray.length === 0) {
      this.dataArray = new Uint8Array(512);
    }

    // Average left + right channels, then expand 64 → 512 bins (factor of 8)
    let maxVal = 0;
    for (let i = 0; i < 64; i++) {
      const leftVal = Math.min(audioArray[i] || 0, 1.0);
      const rightVal = Math.min(audioArray[i + 64] || 0, 1.0);
      const avg = (leftVal + rightVal) / 2;
      if (avg > maxVal) maxVal = avg;
      const byteVal = Math.min(255, Math.floor(avg * 256));
      const baseIdx = i * 8;
      for (let j = 0; j < 8; j++) {
        if (baseIdx + j < 512) {
          this.dataArray[baseIdx + j] = byteVal;
        }
      }
    }

    // Auto-detect whether system audio is playing
    if (maxVal > 0.005) {
      this.wallpaperSilenceFrames = 0;
      if (!this.usingWallpaperAudio) {
        this.usingWallpaperAudio = true;
      }
      this.isPlaying = true;
    } else {
      this.wallpaperSilenceFrames++;
      if (this.wallpaperSilenceFrames > this.wallpaperSilenceThreshold) {
        this.isPlaying = false;
  ...
  public getAudioData(): AudioData {
    // Allow processing when using Wallpaper Engine audio (no analyser needed)
    if (!this.analyser && !this.usingWallpaperAudio) {
    ...
    if (this.isPlaying) {
      // Use Web Audio API analyser for local files, skip for wallpaper system audio (data fed externally)
      if (this.analyser && !this.usingWallpaperAudio) {
